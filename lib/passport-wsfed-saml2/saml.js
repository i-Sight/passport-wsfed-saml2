...
var SAML = function (options) {
  ...
  if (!this.options.cert && (!this.options.thumbprints || this.options.thumbprints.length === 0)) {
    var errMsg = 'You should set either a base64 encoded certificate or the thumbprints of the signing certificates';
...
SAML.prototype.validateSignature = function (xml, options, callback) {
  ...
  if (!signature) {
    var errMsg = 'Signature is missing (xpath: ' + signaturePath + ')';
    console.error(errMsg);
  ...
  sig.keyInfoProvider = {
    ...
    getKey: function (keyInfo) {
      ...
      if(!keyInfo || keyInfo.length===0){
        if(!options.cert) {
          var errMsg = 'options.cert must be specified for SAMLResponses with no embedded signing certificate';
          console.error(errMsg);
          throw new Error(errMsg);
        }
  ...
  if (!valid) {
    var errMsg = 'Signature check errors: ' + sig.validationErrors;
    console.error(errMsg);
    return callback(new Error(errMsg));
  }
  ...
  if (options.thumbprints) {
    ...
    if (!valid_thumbprint) {
      var errMsg = 'Invalid thumbprint (configured: ' + options.thumbprints.join(', ').toUpperCase() + '. calculated: ' + this.calculatedThumbprint.toUpperCase() + ')';
      console.error(errMsg);
      return callback(new Error(errMsg));
    }
...
SAML.prototype.parseAssertion = function(samlAssertion, callback) {
  ...
  if (samlAssertion.getAttribute('MajorVersion') === '1')
    ...
    var errMsg = 'SAML Assertion version not supported';
    console.error(errMsg);
    return callback(new Error(errMsg), null);
  }

  if (self.options.checkExpiration && !self.validateExpiration(samlAssertion, version)) {
    var errMsgE = 'assertion has expired.';
    console.error(errMsgE);
    return callback(new Error(errMsgE), null);
  }

  if (self.options.checkAudience && !self.validateAudience(samlAssertion, self.options.realm, version)) {
    var errMsgA = 'Audience is invalid. Configured: ' + self.options.realm;
    console.error(errMsgA);
    return callback(new Error(errMsgA), null);
  }
